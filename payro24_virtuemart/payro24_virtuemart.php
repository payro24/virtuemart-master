...
class plgVmPaymentpayro24_virtuemart extends vmPSPlugin
{
    private $http;

    /**
    * plgVmPaymentpayro24_virtuemart constructor.
    * @param $subject
    * @param $config
    ...
    */
    ...
    {
        return $this->createTableSQL('Payment payro24 Table');
    }
    ...
    */
    function plgVmConfirmedOrder($cart, $order)
    {
        ...
        if ($session->isActive('payro24')) {
            $session->clear('payro24');
        }
        $session->set('payro24', $crypt_virtuemartPID);
        ...
        $callback = JURI::root() . 'index.php?option=com_virtuemart&view=pluginresponse&task=pluginresponsereceived&gw=payro24';
    ...
    */
    public function plgVmOnPaymentResponseReceived(&$html)
    {
        ...
        if ($gateway == 'payro24') {
            $session = JFactory::getSession();
            if ($session->isActive('payro24') && $session->get('payro24') != null) {
                $cryptID = $session->get('payro24');
            } else {
                $msg = 'سفارش پیدا نشد';
                $link = JRoute::_(JUri::root() . 'index.php/component/virtuemart/cart', false);
                $app->redirect($link, '<h2>' . $msg . '</h2>', $msgType = 'Error');
                ...
                if (!empty($pid) && !empty($porder_id) && $porder_id == $order_id) {
                    if ($pstatus == 10) {
                        ...
                        if (empty($verify_status) || empty($verify_track_id) || empty($verify_amount) || $verify_amount != $price || $verify_status < 100) {
                            $msg = $this->payro24_get_failed_message($method, $verify_track_id, $order_id);
                            $link = JRoute::_(JUri::root() . 'index.php/component/virtuemart/cart', FALSE);
                            $app->redirect($link, '<h2>' . $msg . '</h2>', $msgType = 'Error');
                        } else {
                            //double spending
                            if ($verify_order_id !== $order_id or $orderInfo->payro24_id !== $result->id) {
                                $this->updateStatus('P', 0, $this->otherStatusMessages(), $id);
                                $this->updateOrderInfo($id, $this->otherStatusMessages());
                                $msg = $this->payro24_get_failed_message($method, $verify_track_id, $order_id, 0);
                                $link = JRoute::_(JUri::root() . 'index.php/component/virtuemart/cart', FALSE);
                                $app->redirect($link, '<h2>' . $msg . '</h2>', $msgType = 'Error');
                            }

                            $msg = $this->payro24_get_success_message($method, $verify_track_id, $order_id, $msgNumber);
                            $html = $this->renderByLayout('payro24_virtuemart', array(
                        ...
                        $msg = $this->payro24_get_failed_message($method, 'فاقد کد تراکنش', $order_id, $msgNumber);
                        $link = JRoute::_(JUri::root() . 'index.php/component/virtuemart/cart', FALSE);
                        $app->redirect($link, '<h2>' . $msg . '</h2>', $msgType = 'Error');
                    }
                } else {
    ...
    */
    protected function getOrderInfo($id)
    {
        ...
        $query->select('*')
            ->from($db->qn('#__virtuemart_payment_plg_payro24_virtuemart'));
    ...
    */
    protected function updateOrderInfo($id, $trackingCode)
    {
        ...
        $query->update($db->qn('#__virtuemart_payment_plg_payro24_virtuemart'));
        $query->set($fields);
    ...
    */
    function payro24_get_success_message($method, $track_id, $order_id)
    {
    ...
    * @return string
    */
    public function payro24_get_failed_message($method, $track_id, $order_id, $msgNumber = null)
    {
